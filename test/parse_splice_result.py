...
prediction['SpliecAI'] = prediction.apply(lambda x: parse_spliceai(x[2]), axis=True)
df = vcf.join(prediction)
df.to_csv(sys.argv[2], sep='\t', index=False, columns=['#CHROM', 'POS', 'REF', 'ALT', 'Effect', 'MaxEntScan', 'dbscSNV', 'SpliecAI'])
